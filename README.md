...
- The find-a-pump folder has all the code contained in it
- `cd find-a-pump` then `npm install` then `npm run dev` will start the project locally
- This is a mono-repo project with a separated frontend/backend apps. When you run `npm run dev` in the find-a-pump folder you will start both front and backend to run at the same time using a package called Turbo.
